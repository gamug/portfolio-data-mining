...
# Stand in for a real connector/httpx client that the monkeypatched discovery
# function never actually touches -- cast() documents that instead of
# constructing real (heavier, network-capable) instances just for typing.
_FAKE_CONNECTOR = cast(BaseConnector, object())
_FAKE_CLIENT = cast(httpx.AsyncClient, object())


@pytest.fixture(autouse=True)
def _restore_discover_one_ticker() -> Iterator[None]:
...
async def test_run_domain_discovery_respects_concurrency_limit(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    in_flight = 0
    max_in_flight = 0

    async def fake_discover_one_ticker(
        company: Company,
        connector: BaseConnector,
        client: httpx.AsyncClient,
        date_range: DateRange,
        extra_fetch: object = None,
    ...
    responses = await _common.run_domain_discovery(
        req, connector=_FAKE_CONNECTOR, client=_FAKE_CLIENT, domain="cnbc.com"
    )
...
async def test_run_domain_discovery_default_concurrency_is_five(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    max_in_flight = 0
    in_flight = 0
    ...
    await _common.run_domain_discovery(
        req, connector=_FAKE_CONNECTOR, client=_FAKE_CLIENT, domain="cnbc.com"
    )
...
async def test_resolve_companies_uses_explicit_tickers_when_given(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fail_if_called(client: httpx.AsyncClient) -> list[Company]:
    ...
    companies = await _common.resolve_companies(req, client=_FAKE_CLIENT)
...
async def test_resolve_companies_falls_back_to_wikipedia_when_tickers_omitted(
    monkeypatch: pytest.MonkeyPatch, req_cls: type[DiscoverRequest] | type[RunAllRequest]
) -> None:
    fake_companies = [Company(ticker="MMM", name="3M", sector="Industrials")]

    async def fake_fetch_sp500(client: httpx.AsyncClient) -> list[Company]:
...
async def test_resolve_companies_empty_tickers_list_also_falls_back(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
...
async def test_resolve_companies_translates_wikipedia_failure_to_502(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    async def fake_fetch_sp500(client: httpx.AsyncClient) -> list[Company]:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await _common.resolve_companies(req, client=_FAKE_CLIENT)
