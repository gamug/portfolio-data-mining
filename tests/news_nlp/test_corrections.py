...
def test_update_sentiment_changes_label_and_refreshes_timestamp(conn):
...
def test_update_sentiment_returns_none_for_missing_article(conn):
    assert corrections.update_sentiment(conn, 999, label="negative") is None


def test_delete_sentiment_removes_row_and_returns_true(conn):
    seed_article(conn, id=1)
    conn.commit()
    _seed_sentiment(conn)
...
def test_delete_sentiment_returns_false_when_missing(conn):
    assert corrections.delete_sentiment(conn, 999) is False


def test_deleted_sentiment_reappears_in_pending_articles(conn):
...
def test_update_entity_changes_fields(conn):
    seed_article(conn, id=1)
    conn.commit()
...
def test_update_entity_returns_none_for_missing_id(conn):
    assert corrections.update_entity(conn, 999, text="x") is None


def test_delete_entity_removes_row(conn):
    seed_article(conn, id=1)
    conn.commit()
    entity_id = _seed_entity(conn)
...
def test_delete_entities_for_article_removes_all_and_returns_count(conn):
    seed_article(conn, id=1)
    conn.commit()
    db.write_entities(
        conn, 1,
