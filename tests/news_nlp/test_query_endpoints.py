...
def test_get_article_detail_includes_summary(client, conn):
    seed_article(conn, id=1, company="3M")
    conn.execute(
        """INSERT INTO article_sentiment (article_id, label, score, positive, negative, neutral, model_name, processed_at)
           VALUES (1, 'positive', 0.9, 0.9, 0.05, 0.05, 'test-model', '2023-01-02T00:00:00Z')"""
...
def test_get_article_detail_summary_none_when_unprocessed(client, conn):
    seed_article(conn, id=1, company="3M")
    conn.execute(
        """INSERT INTO article_sentiment (article_id, label, score, positive, negative, neutral, model_name, processed_at)
           VALUES (1, 'positive', 0.9, 0.9, 0.05, 0.05, 'test-model', '2023-01-02T00:00:00Z')"""
...
def test_get_sector_summaries_lists_all_by_default(client, conn):
...
def test_get_sector_summaries_filters_by_sector(client, conn):
    db.write_sector_summary(
        conn, "Industrials", "Industrial Conglomerates", "2026-08-03", "2026-08-09",
        "Industrials summary.", num_articles=1, num_companies=1, model_name="facebook/bart-large-cnn",
    )
