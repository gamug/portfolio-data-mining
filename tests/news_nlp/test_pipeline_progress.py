...
def test_run_sentiment_stage_reports_empty_progress_without_loading_model(conn, monkeypatch):
...
def test_run_ner_stage_reports_empty_progress_without_loading_model(conn, monkeypatch):
    def fail_if_called(*args, **kwargs):
        raise AssertionError("model should not be loaded when there is nothing to process")
...
def test_ner_model_points_at_published_hub_repo():
