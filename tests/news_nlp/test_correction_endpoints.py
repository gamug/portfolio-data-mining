...
def test_patch_category_updates_label(client, conn):
    seed_article(conn, id=1)
    conn.commit()
    _seed_category(conn)
...
def test_patch_category_404_when_missing(client, conn):
    seed_article(conn, id=1)
    conn.commit()
...
def test_patch_category_422_for_invalid_label(client, conn):
    seed_article(conn, id=1)
    conn.commit()
    _seed_category(conn)
...
def test_delete_category_204_and_removes_row(client, conn):
    seed_article(conn, id=1)
    conn.commit()
    _seed_category(conn)
...
def test_delete_category_404_when_missing(client, conn):
    seed_article(conn, id=1)
    conn.commit()
...
def test_get_category_stats(client, conn):
