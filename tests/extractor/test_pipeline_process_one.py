...
async def test_process_one_returns_ok_article_for_successful_fetch():
    def handler(request):
...
async def test_process_one_marks_failed_on_connection_error():
    def handler(request):
