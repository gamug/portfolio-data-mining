...
track"), which is why it lives in a shared domain package rather than
either one.

Point-in-time membership (an `as_of` date, instead of just "today") is
handled by the sibling `data_mining.universe_history` module, backed by a
...
def list_universe(sector: str | None = None, as_of: date | None = None) -> list[dict]:
    ...
    history in data_mining.universe_history, raising ValueError if that date
    ...
    if as_of is not None:
        ...
        # from ever touching data_mining.universe_history at all.
        from data_mining.universe_history import query_as_of  # noqa: PLC0415
...
def resolve_symbol(query: str, as_of: date | None = None) -> dict | None:
    ...
    data_mining.universe_history), raising ValueError if that date predates the
    backfilled coverage or no backfill has been run yet.
    """
    if as_of is not None:
        from data_mining.universe_history import resolve_as_of  # noqa: PLC0415
